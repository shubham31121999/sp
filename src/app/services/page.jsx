...
const allServices = [
  ...Object.entries(layoutA).map(([slug, data]) => ({ slug, ...data })),
  ...Object.entries(layoutB).map(([slug, data]) => ({ slug, ...data })),
];
    ...
    <div className="max-w-7xl mx-auto py-10 px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {allServices.map(({ slug, hero }) => (
          <div key={slug} className="bg-white rounded-lg shadow-md hover:shadow-xl transition-all p-6 flex flex-col justify-between">
            <div>
